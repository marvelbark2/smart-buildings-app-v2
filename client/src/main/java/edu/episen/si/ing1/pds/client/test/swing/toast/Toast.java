package edu.episen.si.ing1.pds.client.test.swing.toast;
...
public class Toast extends JPanel{
    ...
    private void toast(String message, Color bgColor) {
        ...
        if (toasterBodies.isEmpty()) {
            toasterBody = new Body(panelToToastOn, message, bgColor, STARTING_Y_POS);
            CURRENT_Y_OFFSET.set(STARTING_Y_POS + toasterBody.getHeightOfToast());
        } else {
            toasterBody = new Body(panelToToastOn, message, bgColor, CURRENT_Y_OFFSET.get() + SPACER_DISTANCE);
    ...
    private synchronized void removeToast(Body toasterBody) {
